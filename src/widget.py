...
def mask_account_card(number_of_count: str) -> str:
    ...
    if "Visa Platinum " in number_of_count:
        return "Visa Platinum " + get_mask_card_number(number_of_count[15:])
    elif "Maestro " in number_of_count:
        return "Maestro " + get_mask_card_number(number_of_count[9:])
    elif "MasterCard " in number_of_count:
        return "MasterCard " + get_mask_card_number(number_of_count[12:])
    elif "Visa Classic " in number_of_count:
        return "Visa Classic " + get_mask_card_number(number_of_count[13:])
    elif "Счет " in number_of_count:
        return "Счет " + get_mask_account(number_of_count[5:])
    else:
